...
int expandaln(int argc, const char **argv, const Command& command, bool returnAlnRes) {
    ...
    if (returnAlnRes == false) {
        ...
        evaluer = new EvalueComputation(par.dbSize == 0 ? cReader->getAminoAcidDBSize() : par.dbSize, &subMat, par.gapOpen.values.aminoacid(), par.gapExtend.values.aminoacid(), par.strand == 2);
    }
