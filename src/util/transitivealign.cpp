...
int transitivealign(int argc, const char **argv, const Command &command) {
    ...
    EvalueComputation evaluer(par.dbSize == 0 ? sequenceDbr.getAminoAcidDBSize() : par.dbSize, subMat, par.gapOpen.values.aminoacid(), par.gapExtend.values.aminoacid(), par.strand == 2);
